...
def create_todo_handler(
    ...
    todo: ToDo = create_todo(session=session, todo=todo) # id값 지정된 상태
...
def update_todo_handler(
    ...
    todo: ToDo | None = get_todo_by_todo_id(session=session, todo_id=todo_id) # 접근하려는 todo가 유효한 접근인지 확인 먼저
    if todo:
